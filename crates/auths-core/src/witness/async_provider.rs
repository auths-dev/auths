...
//!
//! ```rust,ignore
//! use auths_core::witness::{AsyncWitnessProvider, Receipt, WitnessError, EventHash};
...
pub trait AsyncWitnessProvider: Send + Sync {
    ...
    ///
    /// This can be used for health checks before attempting operations.
    ///
    /// # Default
    ///
...
impl AsyncWitnessProvider for NoOpAsyncWitness {
    async fn submit_event(
        ...
        Ok(Receipt {
            v: super::receipt::KERI_VERSION.into(),
            t: super::receipt::RECEIPT_TYPE.into(),
            d: Said::new_unchecked("ENoop".into()),
            i: "did:key:noop".into(),
            s: 0,
            a: Said::new_unchecked("ENoop".into()),
            sig: vec![0u8; 64],
        })
    }
    ...
    fn quorum(&self) -> usize {
        0 // No quorum required for no-op
    }
}
