    ...
    match decision {
        ...
        TrustDecision::FirstUse { did, presented_pk } => match policy {
            TrustPolicy::Tofu => {
                ...
                if prompt(&msg) {
                    let curve =
                        auths_crypto::CurveType::from_public_key_len_fallback(presented_pk.len())
        ...
        TrustDecision::RotationVerified { old_pin, proof } => {
            let updated = PinnedIdentity {
                ...
                curve: auths_crypto::CurveType::from_public_key_len_fallback(proof.new_public_key.len())
