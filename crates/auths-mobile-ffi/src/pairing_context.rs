...
// Device-key rotation is a local `rot` event on the device's own KEL,
// not a cross-device ceremony — so no rotation-binding helpers live here.
...
fn normalize_p256_signature_to_raw(bytes: &[u8]) -> Result<[u8; 64], MobileError> {
    ...
    // DER path: delegate to the crate-wide helper so DER→raw lives in
    // exactly one place.
