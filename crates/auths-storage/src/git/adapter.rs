...
impl RegistryBackend for GitRegistryBackend {
    ...
    fn store_org_member(&self, org: &str, member: &Attestation) -> Result<(), RegistryError> {
        ...
        if let Some(index) = &self.index {
            ...
            let issuer_did = match IdentityDID::parse(member.issuer.as_str()) {
                Ok(did) => did,
                Err(e) => {
                    log::warn!(
                        "Skipping index update for org member {} in {}: invalid issuer DID: {}",
                        member.subject,
                        org,
                        e
                    );
                        ...
                        Ok(att) => {
                            ...
                            // Validate issuer matches expected org issuer (hard invariant)
                                ...
                                let actual = match IdentityDID::parse(att.issuer.as_str()) {
                                    ...
                                    Err(_) => {
                                        log::warn!(
                                            "Skipping member {} with unparseable issuer DID: {}",
                                            filename,
                                            att.issuer.as_str()
                                        );
                                        return ControlFlow::Continue(());
    ...
    for org_prefix in &org_prefixes {
        match backend.visit_org_member_attestations(org_prefix, &mut |entry| {
            ...
            if let Ok(att) = &entry.attestation {
                ...
                let issuer_did = match IdentityDID::parse(att.issuer.as_str()) {
                    Ok(did) => did,
                    Err(e) => {
                        log::warn!(
                            "Skipping org member {} in {}: invalid issuer DID: {}",
                            entry.did,
                            org_prefix,
                            e
                        );
