...
| Anchors 404 / the node accepts none, though `/health` is green | a *fresh* witness boots fine on an empty registry (the RT-2 fix), but the anchor role can only resolve a party's keys once that party is **in** the registry — "up" is not yet "useful" | Sync the parties' registry: `sync-registry --from <peer, e.g. https://network.auths.dev> --registry /data/registry`, then anchors from those parties are accepted. A fresh node needs no registry to *start*, only to *anchor for* a given party. |
