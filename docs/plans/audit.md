...
3. **auths-express / auths-fastapi:** are these launch-critical? If not, consider removing them from the release train (Theme 3: automate or shrink) rather than building CI for them now. **DECIDED (2026-06-10): launch-critical — promised for launch.** Both stay on the release train with full automation: version sync (`0_versions.py`), lockfiles (`pnpm-lock.yaml` / `uv.lock`), per-package CI (`express-middleware.yml` / `fastapi-middleware.yml`), and publish workflows — express via `publish-typescript.yml` (build/test now pinned to `pnpm install --frozen-lockfile` so the published artifact matches what CI tested), fastapi via the new `publish-fastapi.yml`. OPS prerequisite: configure a PyPI trusted publisher for the `auths-fastapi` project pointing at that workflow.
4. **Error-code stability:** is the `AUTHS-Exxxx` registry meant to be frozen at launch (i.e., is reassigning one side of E4801 the *last* free reassignment)? Affects whether T2's gate should also forbid code *changes*, not just duplicates. **DECIDED (2026-06-10): frozen — the E4801 reassignment was the last silent one.** `gen-error-docs` now enforces `docs/errors/registry.lock` (357 `code = crate::Type::Variant` bindings): new codes ratchet in automatically; reassigning or removing a code fails the build and CI in both modes until the lock line is hand-edited in the same commit, making every contract change a reviewable diff. Live via the existing `gen-error-docs --check` CI gate.
5. **Performance target for the fleet API:** is there an expected org scale (events in KEL, agents per org)? T11 is clearly needed; whether to also add cross-request KEL caching depends on a number. **DECIDED (2026-06-10): target = orgs of 1,000+ engineers × ~10 agents = 10k+ agents, ~50k org-KEL events, sustained churn.** At that scale the current 3-full-replays-per-request is unusable (~150k event reads/request; pagination doesn't help). Read side tracked in #254 (tip-SAID-keyed snapshot cache upgraded to *incremental advance* — fold only events N+1..M, O(churn delta) steady state, fail-closed preserved). Write side (bulk-onboarding ~20-30k sequential kt=1 KEL appends) tracked in #255.
